...
dolen:
		...
		bits -= op;
		op = ( unsigned )( this.op );
		if( __builtin_expect( op == 0, 1 ) ) {/* literal */
			PUP( out ) = ( unsigned char )( this.val );
		}
		else if( __builtin_expect( op & 16, 1 ) ) {/* length base */
...
dodist:
			...
			bits -= op;
			op = ( unsigned )( this.op );
			if( __builtin_expect( op & 16, 1 ) ) {/* distance base */
				...
				if( dist > op ) {/* see if copy from window */
					...
					if( write == 0 ) {/* very common case */
						...
						if( op < len ) {/* some from window */
							len -= op;
							MEMCOPY_BYTE_BY_BYTE( out + OFF, from + OFF, op );
							out += op;
					...
					MEMCOPY_BYTE_BY_BYTE( out + OFF, from + OFF, len );
					out += len;
					from += len;
				}
				else {
