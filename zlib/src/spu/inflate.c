...
int stream_size;
{
    return inflateInit2_(strm, DEF_WBITS_DECOMPRESS, version, stream_size);
}
	...
	while( 1 ) {
		switch( state->mode ) {
			...
			case TABLE:
				...
				state->mode = LENLENS;
			case LENLENS:
				if( __builtin_expect( ( state->ncode - state->have ) > have, 0 ) ) {/* rare case, input buffer is almost empty */
					...
					while( state->have < state->ncode ) {
						if( bits < ( unsigned int )3 ) {/* NEEDBITS, but do not check have == 0 */
							have--;
							hold += ( unsigned long )( *next++ ) << bits;
							bits += 8;
						}
			...
			case CODELENS:
				while( state->have < state->nlen + state->ndist ) {
					...
					if( __builtin_expect( this.val < 16, 1 ) ) {
						NEEDBITS( this.bits );
						DROPBITS( this.bits );
						...
						p_v_us = ( vector unsigned short* )&( state->lens[state->have] );
						v_us = spu_splats( ( unsigned short )len );
						offset = ( int )p_v_us & 0xf;
						init_copy_amt = 8 - ( offset >> 1 );
						if( __builtin_expect( copy >= ( unsigned int )init_copy_amt, 1 ) ) {/* if loop count is large enough to get benefit from SIMDization */
							state->have += copy;

							*p_v_us = spu_sel( *p_v_us, v_us, spu_rlmaskqwbyte( spu_splats( ( unsigned short )0xffff ), -offset ) );
							p_v_us++;
							copy -= init_copy_amt;
							...
							if( copy ) {
								*p_v_us = spu_sel( v_us, *p_v_us, spu_rlmaskqwbyte( spu_splats( ( unsigned short )0xffff ), -( ( int )copy << 1 ) ) );
								copy = 0;
							}
						}
						else {
				...
				state->mode = LEN;
			case LEN:
				if( __builtin_expect( have >= 6 && left >= 258, 0 ) ) {
					RESTORE();
				...
				if( __builtin_expect( this.op && ( this.op & 0xf0 ) == 0, 0 ) ) {
					last = this;
					while( 1 ) {
			...
			case MATCH:
				...
				left -= copy;
				state->length -= copy;
				MEMCOPY_BYTE_BY_BYTE( put, from, copy );
				put += copy;
				from += copy;
				copy = 0;
				if( state->length == 0 ) {
