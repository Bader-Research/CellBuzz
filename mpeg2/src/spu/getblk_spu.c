...
void Decode_MPEG1_Intra_Block (int comp, int dc_dct_pred[])
{
  ...
  for (i=1; ; i++)
  {
    ...
    j = scan[ZIG_ZAG][i];
    val = (val*spu_ld->quantizer_scale*spu_ld->intra_quantizer_matrix[j]) >> 3;
...
void Decode_MPEG1_Non_Intra_Block(int comp)
{
  ...
  for (i=0; ; i++)
  {
    ...
    j = scan[ZIG_ZAG][i];
    val = (((val<<1)+1)*spu_ld->quantizer_scale*spu_ld->non_intra_quantizer_matrix[j]) >> 4;
...
void Decode_MPEG2_Intra_Block (int comp, int dc_dct_pred[])
{
  ...
  */
  ld1 = spu_ld;
  //ld1 = (spu_ld->scalable_mode==SC_DP) ? base : spu_ld;
  bp = ld1->block[comp];
  ...
  qmat = (comp<4 || jobData.chroma_format==CHROMA420)
  ...
  bp[0] = val << (3-jobData.intra_dc_precision);
  ...
  {
    code = Show_Bits(16);
    if (code>=16384 && !jobData.intra_vlc_format)
      tab = &DCTtabnext[(code>>12)-4];
    else if (code>=1024)
    {
      if (jobData.intra_vlc_format)
