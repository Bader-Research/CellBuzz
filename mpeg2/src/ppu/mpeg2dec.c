...
char *argv[];
{
  ...
  if ((baseData.Infile=open(Main_Bitstream_Filename,O_RDONLY|O_BINARY))<0)
  {
...
static void Initialize_Sequence()
{
  ...
  for (cc=0; cc<3; cc++)
  {
    ...
    if (!(backward_reference_frame[cc] = (unsigned char *)malloc_align(size, 4)))
      Error("backward_reference_frame[] malloc failed\n");

    if (!(forward_reference_frame[cc] = (unsigned char *)malloc_align(size, 4)))
      Error("forward_reference_frame[] malloc failed\n");

    if (!(auxframe[cc] = (unsigned char *)malloc_align(size, 4)))
...
Options: -b  file  main bitstream (base)\n\
         -sn       number of SPEs to use\n\
         ...
         -on file  output format (0:YUV 1:SIF 2:TGA 3:PPM 4:X11 5:X11HiQ 6:None)\n\
...
static void Deinitialize_Sequence()
{
  ...
  for(i=0;i<3;i++)
  {
    free_align(backward_reference_frame[i]);
    free_align(forward_reference_frame[i]);
    free_align(auxframe[i]);
...
int *Bitstream_Framenumber;
{
  ...
  if (!Quiet_Flag && runtime!=0)
    printf("%d.%02d seconds, %d frames, %d.%02d fps\n", runtime/100, 
	   runtime%100,Bitstream_Framenum, ((10000*Bitstream_Framenum+runtime/2)/runtime)/100,
	   ((10000*Bitstream_Framenum+runtime/2)/runtime)%100);
#endif
